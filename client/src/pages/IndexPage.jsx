...
function IndexPage() {
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      setIsTyping(true);
      setTimeout(() => {
        setCurrentMessage((prev) => (prev + 1) % realChatMessages.length);
        setIsTyping(false);
      }, 800);
    }, 3000);
      ...
      <div className="min-h-screen bg-gradient-to-br from-gray-900 via-slate-800 to-gray-900 text-white overflow-x-hidden">
        ...
        <section className="px-6 pt-15 md:pt-20 mt-20">
          <div className="max-w-6xl mx-auto">
            ...
            {/* Live Chat Demo yang lebih realistic */}
            <div className="max-w-4xl mx-auto">
              <div className="bg-gray-800/40 backdrop-blur-xl border border-gray-700/50 rounded-3xl p-6 shadow-2xl">
                ...
                <div className="space-y-4 h-96 overflow-y-auto mb-6">
                  {realChatMessages.slice(0, currentMessage + 1).map((message, index) => (
                    <div key={index} className={`flex gap-3 ${index % 2 === 0 ? 'justify-start' : 'justify-end'}`}>
                      {index % 2 === 0 && <div className="w-10 h-10 bg-gradient-to-r from-orange-400 to-red-500 rounded-xl flex items-center justify-center text-white font-bold text-sm flex-shrink-0">{message.avatar}</div>}
                      <div className={`max-w-sm px-4 py-3 rounded-2xl ${index % 2 === 0 ? 'bg-gray-700/60 border border-gray-600/30' : 'bg-gradient-to-r from-blue-500 to-purple-600 ml-auto'}`}>
                        {index % 2 === 0 && <div className="text-xs text-blue-300 font-semibold mb-1">{message.sender}</div>}
                        <div className="text-sm leading-relaxed">{message.text}</div>
                        <div className={`text-xs mt-1 ${index % 2 === 0 ? 'text-gray-400' : 'text-blue-200'}`}>{new Date().toLocaleTimeString('id-ID', { hour: '2-digit', minute: '2-digit' })}</div>
                      </div>
                      {index % 2 === 1 && <div className="w-10 h-10 bg-gradient-to-r from-purple-400 to-pink-500 rounded-xl flex items-center justify-center text-white font-bold text-sm flex-shrink-0">{message.avatar}</div>}
                    </div>
                  ))}
        ...
        {/* Why Section yang lebih personal */}
          ...
          <div className="max-w-4xl mx-auto">
            ...
            <div className="space-y-4">
              {[
                ...
                <details key={index} className="group bg-gray-800/40 border border-gray-700/50 rounded-2xl overflow-hidden">
